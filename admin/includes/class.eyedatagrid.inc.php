...
class EyeDataGrid
{
	...
	const TXT_NORESULTS = 'No hay datos';
	...
	*/
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		echo '<tfoot><tr class="tbl-footer"><td class="tbl-nav" colspan="' . $this->column_count . '"><table width="100%" class="tbl-footer"><tr><td width="10%" class="tbl-found">Registros: <em>' . $this->row_count . '</em>';

		if ($this->row_count > 0)
			//echo ', mostrando <em>' . $first . '</em> hasta <em>' . $last . '</em>';

		echo '</td><td width="80%" class="tbl-pages">';
      ...
      for ($i = $startpage; $i <= $endpage; $i++)
		...
		echo '</td><td width="10%" class="tbl-page">';
	...
	*/
		...
		if ($this->row_count == 0)
			...
			while ($row = $this->_db->fetchAssoc($this->result))
			{
				...
				foreach ($row as $key => $value)
				{
					...
					// le ponemos width fijo a FORO y MENSAJE
					if ($numCol == 1){
						$classWidth = "";
					}else if ($numCol == 2){
						$classWidth = "120px"; // nombre
					}else if ($numCol == 3){
						$classWidth = "120px"; // fecha
					}else if ($numCol == 4){
						$classWidth = "150px"; // foro
					}else if ($numCol == 5){
						$classWidth = "300px"; // mensaje
					}else if ($numCol == 6){
						$classWidth = "70px"; // link
					}else{
						$classWidth = "";
					}
					
					if ($numCol == 4)
						echo '<td class="tbl-cell" style="width: '.$classWidth.'">' . htmlentities(str_replace("&amp;","&",$value)) . '</td>';
					else if ($numCol == 5)
						echo '<td class="tbl-cell" style="width: '.$classWidth.'">' . $this->getNombreTopic($value) . '</td>';
					else if (strpos($value, "http://") > 0 || strpos($value, "img") )
						echo '<td class="tbl-cell" style="width: '.$classWidth.'">' . $value . '</td>';
					else
						echo '<td class="tbl-cell" style="width: '.$classWidth.'">' . htmlentities($value) . '</td>';
				}
	...
	public function getNombreTopic($topicId){
		if ($topicId > 0){
			$mensaje = "Subforo privado";
			$queryTopic = "select CASE WHEN length(post_subject) > 45 THEN CONCAT(substr(post_subject,1,45),'...') ELSE post_subject END as Mensaje
			 				from phpbb_posts where topic_id = ".$topicId." AND forum_id NOT IN (18,33,30,24,31,49,48,50)";
			$this->resultTopic = $this->_db->query($queryTopic, false);
			
				while ($rowTopic = $this->_db->fetchOne($this->resultTopic))
				{
					
					$mensaje = htmlentities($rowTopic);
				}
		}
